...
- **核对按复杂度取舍（8/29 用户纠正）**：不是每次 Trae 改完都核对。简单/低风险改动（纯 View、机械增删、单一效果类、配置默认值）只看 Trae 汇报 + 抽查关键点即可；复杂改动（BattleController 状态机/伤害管道/回合流转/被动接口/多文件联动/字段增删重构）才逐行读源码 + 追踪调用链 + 交叉验证
...
2. 城邦系统：框架闭环，6 城邦里 3 个机制已落地（贸易/季风/元素），3 个待实现（欢愉二档/邪疑三档/战争三档）
3. 8/9 棋子用 Cube 占位
4. 无音效
5. GetAllPassives 每次分配新 List（GC 风险）
6. ~~两个第三方插件嵌套 .git 未追踪~~（已解决 8-29：删嵌套 .git + git rm --cached 清 gitlink，主仓库追踪源码）
...
**当前实现状态**：防御维度已对（物理受防御、魔法/真实不受，见 GetTotalDamageReduction 按 kind 筛选）；护盾维度已实现（8-20 层数护盾：物理受防御+护盾、魔法仅护盾、真实无视两者，见「城邦系统设计」节末行）。
